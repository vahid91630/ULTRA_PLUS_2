...
DISABLE_EXT = os.getenv("EXT_DISABLE","false").lower() in {"1","true","yes","on"}
...
def main():
    ...
    for env_over, cmd in COMMANDS: PROCS.append(spawn(env_over, cmd))
    while True:
        for i, p in enumerate(list(PROCS)):
            ...
            if ret is not None:
                ...
                time.sleep(3); PROCS[i] = spawn(COMMANDS[i][0], COMMANDS[i][1])
        time.sleep(2)
